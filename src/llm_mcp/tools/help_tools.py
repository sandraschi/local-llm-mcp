...
def get_tool_category(tool_name: str) -> ToolCategory:
    ...
    elif tool_name in ['list_models', 'get_model_info', 'register_model', 'generate_text', 'chat_completion', 'embed_text']:
    ...
    issues = {
        ...
        "llm_huggingface_tool": [
            ...
            },
            {
                ...
                "solution": "Use faster internet connection or download smaller models"
            }
        ],
        "gpu_status": [
            {
...
async def _search_tools_impl(mcp: Any, query: str, category: Optional[str] = None) -> Dict[str, Any]:
    ...
    for name, tool in mcp_tools.items():
        ...
        if (query in name.lower() or
            query in doc or
            any(query in param.get("name", "").lower() for param in get_parameter_docs(tool))):
...
async def _get_workflow_guides_impl(category: Optional[str] = None) -> Dict[str, Any]:
...
async def _get_troubleshooting_guide_impl(category: Optional[str] = None) -> Dict[str, Any]:
    ...
    if category:
        troubleshooting = {k: v for k, v in all_troubleshooting.items()
...
def register_help_tools(mcp, register_individual_tools: bool = True):
    ...
    if register_individual_tools:
        ...
        @mcp.tool()
        async def get_workflow_guides(category: Optional[str] = None) -> Dict[str, Any]:
        ...
        @mcp.tool()
        async def get_troubleshooting_guide(category: Optional[str] = None) -> Dict[str, Any]:
