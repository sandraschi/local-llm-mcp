...
def tool_to_schema(tool_func: Callable) -> dict[str, Any]:
    ...
    for name, param in sig.parameters.items():
        ...
        param_schema = {"type": param_type, "description": param_descriptions.get(name, "")}
...
class ToolCallingMixin:
    ...
    async def execute_tool_call(self, tool_call: ToolCall | dict) -> ToolCallResult:
        ...
        try:
            ...
            return ToolCallResult(call_id=tool_call.id or "", name=tool_call.name, content=result)

        except Exception as e:
            logger.exception(f"Error executing tool {tool_call.name}")
            return ToolCallResult(
                call_id=tool_call.id or "", name=tool_call.name, content=f"Error: {e!s}", is_error=True
            )
