...
async def dora_prepare_for_training_impl(
    ...
    try:
        ...
        return {"status": "success", "message": f"Model {model_id} prepared for training", "config": training_args}

    except Exception as e:
        logger.error(f"Error preparing model for training: {e!s}", exc_info=True)
        return {"status": "error", "message": f"Failed to prepare model for training: {e!s}"}


async def dora_train_impl(
    model_id: str, train_dataset: Any, eval_dataset: Any | None = None, **training_kwargs
) -> dict[str, Any]:
    """Train a DoRA model.
    ...
    try:
        ...
        return {"status": "success", "metrics": metrics, "output_dir": model_info["output_dir"]}
