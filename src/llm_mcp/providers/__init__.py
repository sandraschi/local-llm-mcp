...
PROVIDER_CLASSES = {
    ...
    "openrouter": "llm_mcp.providers.openrouter.provider.OpenRouterProvider",
}
...
class ProviderFactory:
    ...
    def create_provider(provider_name: str, config: dict[str, Any]) -> BaseProvider:
        ...
        if isinstance(provider_class_or_path, str):
            try:
                ...
                logger.error(f"Failed to import provider {provider_name} from {provider_class_or_path}: {e!s}")
                raise ImportError(f"Failed to import provider {provider_name}: {e!s}") from e
        else:
        ...
        if provider_class is None:
            try:
                ...
                logger.debug(f"Failed to import provider {provider_name}: {e!s}")
    ...
    @classmethod
    async def create_and_verify_provider(cls, provider_name: str, config: dict[str, Any]) -> BaseProvider | None:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Failed to list models for provider {provider_name}: {e!s}")
                return None

        except Exception as e:
            logger.error(f"Error creating provider {provider_name}: {e!s}")
            raise
