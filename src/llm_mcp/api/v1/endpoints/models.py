...
async def list_models(
    provider: str | None = None, include_vllm: bool = Query(True, description="Include vLLM models in the results")
) -> list[ModelInfo]:
...
async def get_model(model_name: str, provider: str | None = None) -> ModelInfo:
    ...
    try:
        ...
        try:
            ...
            if not provider_instance:
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Model not found: {model_name}")
