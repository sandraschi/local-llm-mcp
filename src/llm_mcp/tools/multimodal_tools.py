...
class MultimodalTools:
    ...
    async def analyze_image(
        self, image: ImageInput, model_name: str = "Salesforce/blip2-opt-2.7b", **kwargs
    ) -> ImageAnalysisResult:
        ...
        return ImageAnalysisResult(
            description=description, tags=tags, metadata={"model": model_name, "device": self.device}
        )

    async def generate_image(
        self, prompt: str, model_name: str = "stabilityai/stable-diffusion-xl-base-1.0", **kwargs
    ) -> GeneratedImage:
        ...
        if not HAS_MM_DEPS:
            raise ImportError(
                "Multimodal dependencies not installed. Install with 'pip install diffusers transformers torch"
            )
        ...
        if self.text_to_image_model is None or self.text_to_image_model.name_or_path != model_name:
            self.text_to_image_model = StableDiffusionPipeline.from_pretrained(
                model_name, torch_dtype=torch.float16 if self.device == "cuda" else torch.float32, use_safetensors=True
            ).to(self.device)
        ...
        return GeneratedImage(
            ...
            mime_type="image/png",
            metadata={"model": model_name, "prompt": prompt, **kwargs},
        )

    async def image_similarity(
        self, image1: ImageInput, image2: ImageInput, model_name: str = "clip-ViT-B-32"
    ) -> float:
    ...
    def _generate_tags(self, text: str) -> list[str]:
        ...
        tags = [word for word, _ in word_counts.most_common(10) if word not in common_words and len(word) > 2]
...
def register_multimodal_tools(mcp):
    ...
    @mcp.tool()
    async def analyze_image(image: str, model_name: str = "Salesforce/blip2-opt-2.7b") -> dict[str, Any]:
        """Analyze an image and generate a description.
    ...
    async def generate_image(
        ...
        return {"image": f"data:{result.mime_type};base64,{image_b64}", "metadata": result.metadata}

    @mcp.tool()
    async def compare_images(image1: str, image2: str, model_name: str = "clip-ViT-B-32") -> dict[str, float]:
...
multimodal_tools_instance = MultimodalTools()


# Standalone functions for portmanteau tool imports
async def analyze_image_impl(image: str, model_name: str = "Salesforce/blip-image-captioning-base") -> dict[str, Any]:
    """Standalone function for image analysis."""
    result = await multimodal_tools_instance.analyze_image(image, model_name=model_name)
    return {"description": result.description, "tags": result.tags, "metadata": result.metadata}
...
async def generate_image_impl(
    ...
    """Standalone function for image generation."""
    result = await multimodal_tools_instance.generate_image(
        prompt=prompt,
        model_name=model_name,
        negative_prompt=negative_prompt,
        width=width,
...
async def image_similarity_impl(image1: str, image2: str, model_name: str = "clip-ViT-B-32") -> float:
    """Standalone function for image similarity."""
    return await multimodal_tools_instance.image_similarity(image1, image2, model_name=model_name)
