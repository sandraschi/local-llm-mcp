...
class OllamaProvider(BaseProvider):
    ...
    async def generate_text(self, model_id: str, prompt: str, **kwargs) -> str:
        ...
        try:
            ...
            response = await self._request("POST", "/api/generate", json=data, timeout=300.0)

            return response.get("response", "")
        except Exception as e:
            raise Exception(f"Failed to generate text with model {model_id}: {e!s}") from e
    ...
    async def chat(self, model_id: str, messages: list[dict[str, str]], **kwargs) -> str:
        ...
        try:
            ...
            response = await self._request("POST", "/api/chat", json=data, timeout=300.0)
            ...
            raise Exception(f"Failed to chat with model {model_id}: {e!s}") from e
    ...
    async def generate_embeddings(self, model_id: str, texts: list[str], **kwargs) -> list[list[float]]:
        ...
        try:
            ...
            embeddings = []
            for text in texts:
                data = {"model": model_id, "prompt": text, **kwargs}

                response = await self._request("POST", "/api/embeddings", json=data, timeout=60.0)
            ...
            raise Exception(f"Failed to generate embeddings with model {model_id}: {e!s}") from e
