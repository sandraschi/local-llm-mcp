...
class MCPServerManager:
        ...
        try:
            ...
            if os.path.exists(self.config_path):
                with open(self.config_path) as f:
                    data = json.load(f)
                    self.servers = {name: MCPServerConfig(**config) for name, config in data.items()}
    ...
    async def _register_server_tools(self, mcp: FastMCP, config: MCPServerConfig):
        ...
        async def get_server_status() -> dict[str, Any]:
            ...
            return {"name": config.name, "type": config.server_type, "enabled": config.enabled, "status": "running"}
