...
import type { LinkHealth } from "@/api/client";
import { getHealth, getLmLinkHealth, listModels } from "@/api/client";
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        try {
          const link = await getLmLinkHealth();
          if (!cancelled) setLinkHealth(link);
        } catch {
          // LM Link optional
        }
      } catch (e) {
  ...
  return (
    <div className="space-y-8 page-enter">
      <div className="flex items-center justify-between">
        ...
        {loading && <div className="text-slate-400 text-sm">Connecting to backend...</div>}
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <Card className="glass-card">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
            <CardTitle className="text-sm font-medium text-slate-300">Available Models</CardTitle>
        ...
        <Card className="glass-card">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
            <CardTitle className="text-sm font-medium text-slate-300">Inference State</CardTitle>
            <div className="p-2 rounded-lg bg-blue-500/10">
              <Radio className="h-4 w-4 text-blue-400" />
            </div>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold">{isConnected ? "Active" : "Idle"}</div>
            <p className="text-xs text-slate-500 mt-1">Local GPU Acceleration Enabled</p>
          </CardContent>
        </Card>

        <Card className="glass-card">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
            <CardTitle className="text-sm font-medium text-slate-300">Handler Pulse</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-500 mt-1">Inference-ready handlers</p>
          </CardContent>
        </Card>

        <Card className="glass-card">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
            <CardTitle className="text-sm font-medium text-slate-300">Compute Load</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-500 mt-1 line-clamp-1">Quantized weights active</p>
          </CardContent>
        </Card>
      </div>

      {/* LM Link peers */}
      {linkHealth?.ok && linkHealth.peers.length > 0 && (
        <Card className="glass-card" data-testid="lm-link-card">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
            <div className="flex items-center gap-2">
              <Link2 className="h-5 w-5 text-blue-400" />
              <CardTitle className="text-sm font-medium text-slate-300">LM Link Peers</CardTitle>
            </div>
            <Badge
              variant={linkHealth.enabled ? "default" : "destructive"}
              className="text-xs"
              data-testid="lm-link-state"
            >
              {linkHealth.enabled ? (
              ...
              {linkHealth.device_name} — {linkHealth.peer_count} peer
              {linkHealth.peer_count !== 1 ? "s" : ""}
            </Badge>
          </CardHeader>
          <CardContent>
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
              {linkHealth.peers.map((peer, i) => {
                const models = peer.models ?? peer.loaded_models ?? [];
                const name = peer.device_name ?? peer.name ?? `Peer ${i + 1}`;
                return (
                  <div
                    key={name}
                    className="flex flex-col gap-2 rounded border border-slate-800 bg-slate-900/50 p-3"
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      {peer.preferred && (
                        <Badge
                          variant="outline"
                          className="text-amber-400 border-amber-800 text-xs ml-auto"
                        >
      ...
      {linkHealth?.ok && linkHealth.peers.length === 0 && (
        <Card className="glass-card border-slate-800/50" data-testid="lm-link-empty">
          <CardContent className="py-4 text-sm text-slate-500 flex items-center gap-2">
            <Link2 className="h-4 w-4" />
            LM Link enabled but no peers connected. Add a device from LM Studio.
          </CardContent>
        </Card>
      )}
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-7">
        <Card className="col-span-4 glass-card">
          <CardHeader>
            <CardTitle className="text-xl font-bold uppercase tracking-widest text-emerald-500/70">
          ...
          <CardContent>
            <div className="h-[250px] font-mono text-sm p-5 overflow-y-auto border border-white/[0.06] rounded-xl bg-black/40 text-slate-400 space-y-1.5 scrollbar-thin scrollbar-thumb-white/10">
              <p className="text-emerald-400">
                [engine] Backend {isConnected ? "connected" : "disconnected"}
              </p>
