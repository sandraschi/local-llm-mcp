...
class GradioManager:
    ...
    def create_custom_interface(
        self, name: str, interface_fn: Callable, inputs: Any | None = None, outputs: Any | None = None, **kwargs
    ) -> dict[str, Any]:
    ...
    def launch_interface(
        ...
        **kwargs,
        ...
        """
        if not GRADIO_AVAILABLE:
            return {"status": "error", "message": "Gradio is not installed. Install with: pip install gradio"}

        if name not in self.interfaces:
            return {"status": "error", "message": f"Interface '{name}' not found"}
                ...
                if config["type"] == "chat":
                    ...
                    with gr.Blocks(title=config["title"], theme=config["theme"]) as demo:
                        ...
                        async def respond(message, chat_history):
                            ...
                            return {chatbot: chat_history, tool_output: tool_calls}

                        # Connect components
                        msg.submit(respond, [msg, chatbot], [chatbot, tool_output])
        ...
        thread = threading.Thread(target=run_interface, daemon=True, name=f"gradio-{name}")
        thread.start()
    ...
    def list_interfaces(self) -> dict[str, Any]:
        ...
        return {
            name: {
                ...
                "config": {k: v for k, v in config.items() if k not in ["generate_fn", "interface_fn"]},
            }
...
async def _create_chat_interface_impl(
    ...
    **kwargs,
    ...
    async def generate_fn(message: str, history: list[tuple] | None = None) -> dict[str, Any]:
        history = history or []
        ...
        try:
            ...
            response = {"content": f"Echo: {message}", "tool_calls": []}
            ...
            return {"content": f"Error generating response: {e!s}", "tool_calls": []}

    return _gradio_manager.create_chat_interface(
        name=name, generate_fn=generate_fn, title=title, description=description, theme=theme, **kwargs
    )
    ...
    async def gradio_create_chat_interface(
        ...
        **kwargs,
        ...
        return await _create_chat_interface_impl(
            mcp=mcp, name=name, title=title, description=description, theme=theme, tools=tools, **kwargs
        )

    @mcp.tool()
    async def gradio_launch_interface(
        name: str, server_name: str = "0.0.0.0", server_port: int = 7860, share: bool = False, **kwargs
    ) -> dict[str, Any]:
        """Launch a Gradio interface.
        ...
        return _gradio_manager.launch_interface(
            name=name, server_name=server_name, server_port=server_port, share=share, **kwargs
        )
