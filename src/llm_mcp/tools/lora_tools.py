...
class LoraManager:
    ...
    def __init__(self, base_model=None, lora_dir: str | None = None):
    ...
    def load_adapter(self, adapter_name: str, adapter_path: str | None = None, **kwargs) -> dict[str, Any]:
        """Load a LoRA adapter.
        ...
        if not PEFT_AVAILABLE:
            return {"status": "error", "message": "PEFT library is not installed. Install with: pip install peft"}

        if not self.base_model:
            return {"status": "error", "message": "No base model set. Call set_base_model() first."}
        ...
        try:
            ...
            # Load the adapter
            model = PeftModel.from_pretrained(self.base_model, adapter_path, adapter_name=adapter_name, **kwargs)

            # Store adapter info
            config = PeftConfig.from_pretrained(adapter_path)
            self.loaded_adapters[adapter_name] = {"model": model, "config": config.to_dict(), "path": adapter_path}

            # Save updated config
            self._save_config()

            return {"status": "success", "adapter": adapter_name, "config": config.to_dict()}

        except Exception as e:
            logger.error(f"Failed to load adapter {adapter_name}: {e}")
            return {"status": "error", "message": f"Failed to load adapter: {e!s}"}
...
async def _lora_list_adapters_impl(adapter_dir: str | None = None) -> list[dict[str, Any]]:
...
async def _lora_load_adapter_impl(
    adapter_name: str, adapter_path: str | None = None, base_model: str | None = None, **kwargs
) -> dict[str, Any]:
...
def register_lora_tools(mcp):
    ...
    @mcp.tool()
    async def lora_list_adapters(adapter_dir: str | None = None) -> list[dict[str, Any]]:
        """List all available LoRA adapters.
