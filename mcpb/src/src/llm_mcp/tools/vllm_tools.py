...
if VLLM_AVAILABLE:
    logger.info("vLLM imports successful", multimodal=MULTIMODAL_AVAILABLE)
else:
...
# Global instance for vLLM manager
...
async def _vllm_initialize_impl(model_name: str, **kwargs) -> Dict[str, Any]:
    ...
    manager = get_vllm_manager()
    try:
        config = VLLMModelConfig(model_name=model_name, **kwargs)
...
def register_vllm_tools(mcp):
    """Register vLLM tools with the MCP server."""
    manager = get_vllm_manager()
