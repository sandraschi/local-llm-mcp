...
def register_tool[T](
    ...
    decorator = mcp.tool(name=tool_name, description=description, stateful=stateful, ttl=ttl)

    # Apply the decorator to the function
    decorated_func = decorator(func)

    # Log the registration
    logger.debug(f"Registered tool: {tool_name} (stateful={stateful}, ttl={ttl or 'default'})")
