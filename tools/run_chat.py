...
class ChatApplication:
    ...
    async def _process_chat_input(self, user_input: str):
        ...
        if not self.terminal.provider or not self.terminal.model:
            print(
                f"{Colors.YELLOW}Please set a provider and model first "
                f"using /provider and /model commands.{Colors.ENDC}"
            )
            return
        ...
        try:
            ...
            print(
                f"\n{Colors.CYAN}Generated {token_count} tokens in "
                f"{duration:.1f}s ({tokens_per_second:.1f} tokens/s){Colors.ENDC}"
            )
            ...
            error_msg = f"{Colors.RED}Error ({error_type}): {e!s}{Colors.ENDC}"
