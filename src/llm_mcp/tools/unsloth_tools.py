...
class UnslothManager:
    ...
    def load_model(
        ...
        **kwargs,
        ...
        if not UNLOTH_AVAILABLE:
            raise ImportError(
                "Unsloth is not installed. Install with: pip install git+https://github.com/unslothai/unsloth.git"
            )
    ...
    def prepare_for_training(self, model_id: str, training_config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    def train(
        ...
        return {"status": "success", "metrics": train_result.metrics, "output_dir": output_dir}
    ...
    def list_models(self) -> dict[str, Any]:
        ...
        """
        return {
            model_id: {"config": model_info["config"], "device": str(model_info["model"].device)}
...
def register_unsloth_tools(mcp):
    ...
    async def unsloth_load_model(
        model_name: str, max_seq_length: int = 2048, load_in_4bit: bool = True, token: str | None = None, **kwargs
    ) -> dict[str, Any]:
        """Load a model with Unsloth optimizations.
        ...
        return unsloth_manager.load_model(
            model_name=model_name, max_seq_length=max_seq_length, load_in_4bit=load_in_4bit, token=token, **kwargs
        )

    @mcp.tool()
    async def unsloth_prepare_for_training(
        model_id: str, training_config: dict[str, Any] | None = None
    ) -> dict[str, Any]:
