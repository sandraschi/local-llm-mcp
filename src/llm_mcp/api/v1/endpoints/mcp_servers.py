...
async def list_mcp_servers(enabled_only: bool = True) -> list[MCPServerStatus]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to list MCP servers: {e!s}"
        )
...
async def get_mcp_server(server_name: str) -> MCPServer:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get MCP server: {e!s}"
        )


@router.put("/mcp-servers/{server_name}", response_model=MCPServer)
async def update_mcp_server(server_name: str, server_update: MCPServerUpdate) -> MCPServer:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to update MCP server: {e!s}"
        )
...
async def delete_mcp_server(server_name: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to delete MCP server: {e!s}"
        )
...
async def start_mcp_server(server_name: str) -> MCPServerOperation:
    ...
    try:
        ...
        if not server.enabled:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=f"Cannot start disabled MCP server '{server_name}'"
            )
        ...
        if not success:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to start MCP server '{server_name}'"
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to start MCP server: {e!s}"
        )
...
async def stop_mcp_server(server_name: str) -> MCPServerOperation:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to stop MCP server: {e!s}"
        )
...
async def get_mcp_server_status(server_name: str) -> MCPServerStatus:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get MCP server status: {e!s}"
        )


@router.get("/mcp-servers/{server_name}/logs", response_model=MCPServerLogs)
async def get_mcp_server_logs(server_name: str, limit: int = 100, next_token: str | None = None) -> MCPServerLogs:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get MCP server logs: {e!s}"
        )
...
async def discover_available_servers() -> list[MCPServerDiscovery]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to discover available MCP servers: {e!s}"
        )
