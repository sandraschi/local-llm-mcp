...
def setup_mcp(mcp: FastMCP) -> None:
    ...
    async def unload_model(model_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                if success
                else f"Failed to unload model {model_id}",
            }
        except Exception as e:
            return {"success": False, "model_id": model_id, "error": str(e)}
    ...
    async def get_provider_status(provider_name: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not provider_type:
                return {"provider": provider_name, "status": "error", "error": f"Provider '{provider_name}' not found"}
            ...
            if provider:
                ...
                return {
                    ...
                    "initialized": True,
                    "details": {
                        "model_count": len(await provider.list_models())
                ...
                return {"provider": provider_name, "status": "not_initialized", "initialized": False}
    ...
    async def load_provider(
        provider_name: str, auto_start: bool = True, wait_until_ready: bool = True, timeout: int = 30
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if not provider_type:
                return {"success": False, "provider": provider_name, "error": f"Provider '{provider_name}' not found"}
                        ...
                        try:
                            ...
                            if wait_until_ready:
                                ...
                                while (asyncio.get_event_loop().time() - start_time) < timeout:
                                    try:
                                        async with httpx.AsyncClient() as client:
                                            response = await client.get(urljoin(base_url, "/api/tags"), timeout=2.0)
            ...
            try:
                ...
                return {"success": False, "provider": provider_name, "error": f"Failed to initialize provider: {e!s}"}
