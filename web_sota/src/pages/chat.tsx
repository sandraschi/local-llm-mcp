...
import { type ModelInfo, generate, listModels } from "@/api/client";
...
export function Chat() {
	const modelSelectId = useRef(
		`model-select-${Math.random().toString(36).substr(2, 9)}`,
	).current;
	...
	const personalityColor =
		PERSONALITY_COLORS[personality.color] ?? PERSONALITY_COLORS.slate;
		...
		listModels()
			.then((list) => {
				...
				if (
					d &&
					list.some(
						(m) => (m.id ?? m.name) === d.model && m.provider === d.provider,
					)
				) {
		...
		<div className="flex h-[calc(100vh-8rem)] flex-col space-y-4">
			...
			<Card className="flex-1 border-slate-800 bg-slate-950/50 flex flex-col overflow-hidden min-h-0">
				<CardContent className="flex-1 overflow-y-auto p-4 space-y-4">
					{messages.length === 0 && (
						<p className="text-slate-500 text-sm">
							...
							<span className={cn("font-medium", personalityColor)}>
								{personality.name}
							</span>{" "}
				...
				<form
					...
					className="p-4 border-t border-slate-800 bg-slate-900/30"
				>
					{interimTranscript && (
