...
def get_system_info() -> Dict[str, Any]:
    ...
    if gpu_info["available"]:
        gpu_data = {
            "name": gpu_info["gpu"]["name"],
            "total_gb": gpu_info["gpu"]["total_gb"],
            "free_gb": gpu_info["gpu"]["free_gb"],
            "used_gb": round(gpu_info["gpu"]["used_mb"] / 1024, 1),
            "recommended_mode": gpu_info["recommended_mode"],
            "can_run_full": gpu_info["can_run_full"],
            "can_run_low": gpu_info["can_run_low"],
        }
    else:
        gpu_data = {"status": "No NVIDIA GPU detected"}
...
def register_system_tools(mcp):
    ...
    async def server_restart(delay: int = 0) -> Dict[str, Any]:
        ...
        return {
            ...
            "message": "Restart functionality not implemented",
        }
    ...
    async def server_shutdown(delay: int = 0) -> Dict[str, Any]:
        ...
        if delay > 0:
            return {
                "status": "scheduled",
                "message": f"Server will shut down in {delay} seconds",
            }

        # In a real implementation, this would trigger a shutdown
        return {
            "status": "not_implemented",
            "message": "Shutdown functionality not implemented",
        }
