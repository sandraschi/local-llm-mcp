...
class ModelManager:
    ...
    async def refresh_models(self):
        ...
        async with self._models_lock:
            ...
            for _provider_type, models in all_models.items():
                for model in models:
                    self._models_cache[model.id] = model

    async def list_models(self, provider: str | None = None) -> list[ModelMetadata]:
        ...
        if provider:
            ...
            try:
                ...
                return [model for model in self._models_cache.values() if model.provider == provider_enum]
            except ValueError:
    ...
    async def load_model(self, model_id: str, **kwargs) -> ModelMetadata:
        ...
        try:
            ...
            raise ValueError(f"Error loading model {model_id}: {e!s}") from e
    ...
    async def generate_text(self, model_id: str, prompt: str, **kwargs) -> str:
        ...
        try:
            ...
            raise ValueError(f"Error generating text with model {model_id}: {e!s}") from e
    ...
    async def chat(self, model_id: str, messages: list[dict[str, str]], **kwargs) -> str:
        ...
        try:
            ...
            raise ValueError(f"Error in chat with model {model_id}: {e!s}") from e
