...
from .chat_terminal import ChatTerminal, Colors
...
class CommandProcessor:
    ...
    async def cmd_search(self, args: list[str]) -> None:
        ...
        for idx, (msg_idx, msg) in enumerate(results, 1):
            ...
            print(
                f"{Colors.BLUE}{msg.role.upper()}:{Colors.ENDC} "
    ...
    async def cmd_provider(self, args: list[str]) -> None:
        ...
        if provider_name not in available_providers:
            print(
                f"{Colors.RED}Unknown provider: {provider_name}. "
                f"Available providers: {', '.join(available_providers)}{Colors.ENDC}"
            )
            return
