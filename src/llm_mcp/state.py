    ...
    def update_usage(self, tokens_generated: int, duration_seconds: float):
        ...
        if duration_seconds > 0:
            ...
            if self.average_tokens_per_second == 0:
                ...
                self.average_tokens_per_second = alpha * current_tps + (1 - alpha) * self.average_tokens_per_second
...
class SessionState:
    ...
    def add_message(self, role: str, content: str, model: str | None = None, tokens: int = 0, **metadata):
...
class StateManager:
        ...
        while self._running:
            try:
                ...
                if len(self.sessions) > self.max_sessions:
                    oldest_sessions = sorted(self.sessions.items(), key=lambda x: x[1].last_active)
    ...
    def register_model(self, model_name: str, model_type: str, provider: str = "unknown", **metadata) -> ModelState:
        ...
        model_state = ModelState(model_name=model_name, model_type=model_type, provider=provider, metadata=metadata)
    ...
    def get_model_state(self, model_name: str, provider: str = "unknown") -> ModelState | None:
